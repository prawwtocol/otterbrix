            ...
            switch (expr->type()) {
                ...
                case compare_type::regex: {
                    if (std::holds_alternative<components::expressions::key_t>(expr->left())) {
                        auto& left_key = std::get<components::expressions::key_t>(expr->left());
                        // For join conditions (different schemas), default unqualified left
                        // operand to the left schema to avoid false ambiguity.
                        if (!same_schema && left_key.side() == side_t::undefined) {
                    ...
                    if (std::holds_alternative<components::expressions::key_t>(expr->right())) {
                        auto& right_key = std::get<components::expressions::key_t>(expr->right());
                        // For join conditions, default unqualified right operand to the
                        // right schema to avoid false ambiguity.
                        if (!same_schema && right_key.side() == side_t::undefined) {
        ...
        schema_result<named_schema> validate_schema(std::pmr::memory_resource* resource,
            ...
            if (node->expressions().empty()) {
                ...
                if (node->expressions()[0]->group() == expression_group::compare) {
                    ...
                    auto result = validate_schema(resource, catalog, expr, parameters, schema_left, schema_right, same_schema);
                    return result;
    ...
    schema_result<named_schema> validate_schema(std::pmr::memory_resource* resource,
        ...
        switch (node->type()) {
            ...
            case node_type::join_t: {
                ...
                auto* join_node = static_cast<const components::logical_plan::node_join_t*>(node);
                if (join_node->type() == components::logical_plan::join_type::semi ||
                    join_node->type() == components::logical_plan::join_type::anti) {
