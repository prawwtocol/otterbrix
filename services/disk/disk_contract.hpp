...
namespace services::disk {
    ...
    struct disk_contract {
        ...
        storage_append(session_id_t session,
                       collection_full_name_t name,
                       std::unique_ptr<components::vector::data_chunk_t> data);
        actor_zeta::unique_future<void> storage_update(session_id_t session,
                                                       collection_full_name_t name,
                                                       components::vector::vector_t row_ids,
                                                       std::unique_ptr<components::vector::data_chunk_t> data);
        actor_zeta::unique_future<uint64_t> storage_delete_rows(session_id_t session,
                                                                collection_full_name_t name,
                                                                components::vector::vector_t row_ids,
                                                                uint64_t count);
