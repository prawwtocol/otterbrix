...
namespace otterbrix {
    ...
    base_otterbrix_t::base_otterbrix_t(const configuration::config& config)
        ...
        if (!wal_records.empty()) {
            ...
            trace(log_,
                  "spaces::replaying {} WAL records across {} collection groups",
                  wal_records.size(),
                  grouped_records.size());
            ...
            for (auto& [coll_name, records] : grouped_records) {
                replay_threads.emplace_back([this, &records, &coll_name] {
                    ...
                    for (auto* record : records) {
                        trace(log_,
                              "spaces::replaying WAL record id {} type {} (collection {})",
                              record->id,
                        ...
                        if (cursor->is_error()) {
                            warn(log_,
                                 "spaces::failed to replay WAL record {}: {}",
                                 record->id,
