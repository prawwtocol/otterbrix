...
namespace components::operators {
    ...
    void operator_join_t::on_execute_impl(pipeline::context_t* context) {
        ...
        if (left_->output() && right_->output()) {
            ...
            bool left_only = (join_type_ == type::semi || join_type_ == type::anti);
            ...
            switch (join_type_) {
                ...
                case type::cross:
                    ...
                    break;
                case type::semi:
                    semi_join_(predicate, context);
                    break;
                case type::anti:
                    anti_join_(predicate, context);
                    break;
                default:
                    break;
            }
    ...
    void operator_join_t::semi_join_(const predicates::predicate_ptr& predicate, pipeline::context_t*) {
        const auto& chunk_left = left_->output()->data_chunk();
        const auto& chunk_right = right_->output()->data_chunk();
        auto& chunk_res = output_->data_chunk();
        ...
        for (size_t i = 0; i < chunk_left.size(); i++) {
            for (size_t j = 0; j < chunk_right.size(); j++) {
                if (predicate->check(chunk_left, chunk_right, i, j)) {
                    copy_indices_left.emplace_back(i);
                    ++res_count;
                    break; // only need one match per left row
                }
            }
        }
    ...
    void operator_join_t::anti_join_(const predicates::predicate_ptr& predicate, pipeline::context_t*) {
        const auto& chunk_left = left_->output()->data_chunk();
        const auto& chunk_right = right_->output()->data_chunk();
        auto& chunk_res = output_->data_chunk();
