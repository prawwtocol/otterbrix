...
namespace components::operators::predicates::impl {
    ...
    value_getter create_value_getter(const expressions::key_t& key) {
        // If path is set (by validate_schema), use it for O(1) column access.
        // Otherwise, fall back to searching by column name at runtime.
        // This fallback is needed for raw_data plans that bypass validate_schema.
        if (!key.path().empty()) {
        ...
        // Fallback: resolve column by name at runtime using types() which returns a safe copy
        return [name = key.as_string(), cached_idx = static_cast<size_t>(-1)]
               (const vector::data_chunk_t& chunk_left,
                const vector::data_chunk_t&,
                size_t index_left,
                size_t) mutable -> types::logical_value_t {
            // Cache the column index on first call
            if (cached_idx == static_cast<size_t>(-1)) {
                auto types = chunk_left.types();
                for (size_t i = 0; i < types.size(); ++i) {
                    if (types[i].alias() == name) {
                        cached_idx = i;
                        break;
                    }
                }
                if (cached_idx == static_cast<size_t>(-1)) {
                    throw std::runtime_error("Column '" + name + "' not found in data chunk");
                }
            }
            return chunk_left.data[cached_idx].value(index_left);
        };
    }
