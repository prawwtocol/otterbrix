...
components::vector::data_chunk_t gen_data_chunk(size_t size, int num, std::pmr::memory_resource* resource) {
    ...
    for (size_t i = 1; i <= size; i++) {
        ...
        chunk.set_value(1, i - 1, logical_value_t{resource, std::to_string(static_cast<int64_t>(i) + num)});
        chunk.set_value(2, i - 1, logical_value_t{resource, static_cast<double>(static_cast<int64_t>(i) + num) + 0.1});
        chunk.set_value(3, i - 1, logical_value_t{resource, (static_cast<int64_t>(i) + num) % 2 != 0});
        {
            ...
            chunk.set_value(4, i - 1, logical_value_t::create_array(resource, logical_type::UBIGINT, arr));
        }
    }
