...
namespace {
    ...
    static compute_status count_finalize(kernel_context& ctx, std::pmr::vector<logical_value_t>& out) {
        out.emplace_back(out.get_allocator().resource(), static_cast<uint64_t>(static_cast<count_kernel_state*>(ctx.state())->value));
        return compute_status::ok();
    }
    ...
    double to_double(const logical_value_t& v) {
        switch (v.type().type()) {
            case logical_type::TINYINT:  return static_cast<double>(v.value<int8_t>());
            case logical_type::SMALLINT: return static_cast<double>(v.value<int16_t>());
            case logical_type::INTEGER:  return static_cast<double>(v.value<int32_t>());
            case logical_type::BIGINT:   return static_cast<double>(v.value<int64_t>());
            case logical_type::UTINYINT: return static_cast<double>(v.value<uint8_t>());
            case logical_type::USMALLINT:return static_cast<double>(v.value<uint16_t>());
            case logical_type::UINTEGER: return static_cast<double>(v.value<uint32_t>());
            case logical_type::UBIGINT:  return static_cast<double>(v.value<uint64_t>());
            case logical_type::FLOAT:    return static_cast<double>(v.value<float>());
            case logical_type::DOUBLE:   return v.value<double>();
            default: return 0.0;
        }
    }
    ...
    static compute_status avg_finalize(kernel_context& ctx, std::pmr::vector<logical_value_t>& out) {
        auto* acc = static_cast<avg_kernel_state*>(ctx.state());
        double result = acc->count > 0 ? to_double(acc->value) / static_cast<double>(acc->count) : 0.0;
        out.emplace_back(acc->value.resource(), result);
        return compute_status::ok();
    }
